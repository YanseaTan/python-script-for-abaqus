...
mdb.models['Model-1'].materials['aggregate'].Elastic(table=((55000.0, 0.25), ))
mdb.models['Model-1'].materials['aggregate'].Density(table=((2.5e-9, ), ))
mdb.models['Model-1'].HomogeneousSolidSection(name='aggregate', material='aggregate', thickness=None)
p = mdb.models['Model-1'].parts['Part-Ball-16']
c = p.cells
cells = c.getSequenceFromMask(mask=('[#1 ]', ), )
region = regionToolset.Region(cells=cells)
...
radius = 8
while True:
    disX = random.uniform(radius+0.2, 50-radius-0.2)
    disY = random.uniform(radius+0.2, 50-radius-0.2)
    disZ = random.uniform(radius+0.2, 100-radius-0.2)
    if len(center16)==0:
        center16.append([disX,disY,disZ])
    else:
        if interCheck([disX,disY,disZ],center16,8,8):
            center16.append([disX,disY,disZ])
            count += 1
...
## aggregate
instances1 = []
for index in range(len(center16)):
    myAssembly.Instance(name='Part-Ball-16-{}'.format(index), part=myModel.parts["Part-Ball-16"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-16-{}'.format(index),), vector=tuple(center16[index]))
    instances1.append(myAssembly.instances['Part-Ball-16-{}'.format(index)])
...
for index in range(len(center16)):
    myAssembly.Instance(name='Part-Ball-16-{}'.format(index), part=myModel.parts["Part-Ball-16"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-16-{}'.format(index),), vector=tuple(center16[index]))
    cells1.append(myAssembly.instances['Part-Ball-16-{}'.format(index)].cells[0:8])
    instances2.append(myAssembly.instances['Part-Ball-16-{}'.format(index)])
...
# job
mdb.Job(name='b20t-16', model='Model-1', description='', type=ANALYSIS, 
