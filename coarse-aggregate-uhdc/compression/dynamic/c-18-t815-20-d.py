...
for diameter in [8, 12, 15]:
    ...
    mysketch_2.ArcByCenterEnds(center=(0.0, 0.0), point1=(0.0, diameter/2.0), point2=(diameter/2.0, 0.0), 
        direction=CLOCKWISE)
...
mdb.models['Model-1'].materials['aggregate'].Elastic(table=((30000.0, 0.2), ))
mdb.models['Model-1'].materials['aggregate'].Density(table=((2.5e-09, ), ))
mdb.models['Model-1'].HomogeneousSolidSection(name='aggregate', material='aggregate', thickness=None)
p = mdb.models['Model-1'].parts['Part-Ball-15']
c = p.cells
...
thicknessAssignment=FROM_SECTION)
p = mdb.models['Model-1'].parts['Part-Ball-12']
c = p.cells
cells = c.getSequenceFromMask(mask=('[#ff ]', ), )
region = regionToolset.Region(cells=cells)
p.SectionAssignment(region=region, sectionName='aggregate', offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='',
thicknessAssignment=FROM_SECTION)
p = mdb.models['Model-1'].parts['Part-Ball-8']
c = p.cells
cells = c.getSequenceFromMask(mask=('[#ff ]', ), )
region = regionToolset.Region(cells=cells)
p.SectionAssignment(region=region, sectionName='aggregate', offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='',
...
radius = 7.5
while True:
    disX = random.uniform(radius+1, 100-radius-1)
    disY = random.uniform(radius+1, 100-radius-1)
    disZ = random.uniform(radius+1, 100-radius-1)
    if len(center15)==0:
        center15.append([disX,disY,disZ])
    else:
        if interCheck([disX,disY,disZ],center15,7.5,7.5):
            center15.append([disX,disY,disZ])
            count15 += 1
    if count15 >= 100*100*100*dopingRate*2/3/4*3/math.pi/radius/radius/radius:
        break
...
while True:
    ...
    disY = random.uniform(radius+1, 100-radius-1)
    disZ = random.uniform(radius+1, 100-radius-1)
    if interCheck([disX,disY,disZ],center12,6,6):
        if interCheck([disX,disY,disZ],center15,6,7.5):
            center12.append([disX,disY,disZ])
            count12 += 1
    if count12 >= 100*100*100*dopingRate/4/4*3/math.pi/radius/radius/radius:
        break
...
while True:
    ...
    disY = random.uniform(radius+1, 100-radius-1)
    disZ = random.uniform(radius+1, 100-radius-1)
    if interCheck([disX,disY,disZ],center8,4,4):
        if interCheck([disX,disY,disZ],center12,4,6):
            if interCheck([disX,disY,disZ],center15,4,7.5):
                center8.append([disX,disY,disZ])
                count8 += 1
    if count8 >= 100*100*100*dopingRate/12/4*3/math.pi/radius/radius/radius:
        break
...
## aggregate
instances1 = []
for index in range(len(center15)):
    myAssembly.Instance(name='Part-Ball-15-{}'.format(index), part=myModel.parts["Part-Ball-15"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-15-{}'.format(index),), vector=tuple(center15[index]))
    instances1.append(myAssembly.instances['Part-Ball-15-{}'.format(index)])

for index in range(len(center12)):
    myAssembly.Instance(name='Part-Ball-12-{}'.format(index), part=myModel.parts["Part-Ball-12"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-12-{}'.format(index),), vector=tuple(center12[index]))
    instances1.append(myAssembly.instances['Part-Ball-12-{}'.format(index)])

for index in range(len(center8)):
    myAssembly.Instance(name='Part-Ball-8-{}'.format(index), part=myModel.parts["Part-Ball-8"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-8-{}'.format(index),), vector=tuple(center8[index]))
    instances1.append(myAssembly.instances['Part-Ball-8-{}'.format(index)])
...
for index in range(len(center15)):
    myAssembly.Instance(name='Part-Ball-15-{}'.format(index), part=myModel.parts["Part-Ball-15"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-15-{}'.format(index),), vector=tuple(center15[index]))
    cells1.append(myAssembly.instances['Part-Ball-15-{}'.format(index)].cells[0:8])
    instances2.append(myAssembly.instances['Part-Ball-15-{}'.format(index)])

for index in range(len(center12)):
    myAssembly.Instance(name='Part-Ball-12-{}'.format(index), part=myModel.parts["Part-Ball-12"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-12-{}'.format(index),), vector=tuple(center12[index]))
    cells1.append(myAssembly.instances['Part-Ball-12-{}'.format(index)].cells[0:8])
    instances2.append(myAssembly.instances['Part-Ball-12-{}'.format(index)])

for index in range(len(center8)):
    myAssembly.Instance(name='Part-Ball-8-{}'.format(index), part=myModel.parts["Part-Ball-8"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-8-{}'.format(index),), vector=tuple(center8[index]))
    cells1.append(myAssembly.instances['Part-Ball-8-{}'.format(index)].cells[0:8])
    instances2.append(myAssembly.instances['Part-Ball-8-{}'.format(index)])
...
# job
mdb.Job(name='t20c', model='Model-1', description='', type=ANALYSIS, 
