...
mdb.models['Model-1'].materials['aggregate'].Elastic(table=((8.0, 0.49), ))
mdb.models['Model-1'].materials['aggregate'].Density(table=((9.3e-10, ), ))
...
p.SectionAssignment(region=region, sectionName='aggregate', offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='',
thicknessAssignment=FROM_SECTION)
p = mdb.models['Model-1'].parts['Part-Ball-5']
c = p.cells
cells = c.getSequenceFromMask(mask=('[#ff ]', ), )
region = regionToolset.Region(cells=cells)
p.SectionAssignment(region=region, sectionName='aggregate', offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='',
...
while True:
    ...
    if len(center10)==0:
        ...
        if interCheck([disX,disY,disZ],center10,5,5):
            center10.append([disX,disY,disZ])
            count10 += 1
    if count10 >= 100*100*100*dopingRate*2/3/4*3/math.pi/radius/radius/radius:
        break
...
while True:
    ...
    if interCheck([disX,disY,disZ],center8,4,4):
        if interCheck([disX,disY,disZ],center10,4,5):
            center8.append([disX,disY,disZ])
            count8 += 1
    if count8 >= 100*100*100*dopingRate/4/4*3/math.pi/radius/radius/radius:
        break
...
while True:
    ...
    disY = random.uniform(radius+1, 100-radius-1)
    disZ = random.uniform(radius+1, 100-radius-1)
    if interCheck([disX,disY,disZ],center5,2.5,2.5):
        if interCheck([disX,disY,disZ],center8,2.5,4):
            if interCheck([disX,disY,disZ],center10,2.5,5):
                center5.append([disX,disY,disZ])
                count5 += 1
    if count5 >= 100*100*100*dopingRate/12/4*3/math.pi/radius/radius/radius:
        break
...
for index in range(len(center5)):
    myAssembly.Instance(name='Part-Ball-5-{}'.format(index), part=myModel.parts["Part-Ball-5"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-5-{}'.format(index),), vector=tuple(center5[index]))
    instances1.append(myAssembly.instances['Part-Ball-5-{}'.format(index)])
...
for index in range(len(center5)):
    myAssembly.Instance(name='Part-Ball-5-{}'.format(index), part=myModel.parts["Part-Ball-5"], dependent=ON)
    myAssembly.translate(instanceList=('Part-Ball-5-{}'.format(index),), vector=tuple(center5[index]))
    cells1.append(myAssembly.instances['Part-Ball-5-{}'.format(index)].cells[0:8])
    instances2.append(myAssembly.instances['Part-Ball-5-{}'.format(index)])
...
# job
mdb.Job(name='x20c', model='Model-1', description='', type=ANALYSIS, 
