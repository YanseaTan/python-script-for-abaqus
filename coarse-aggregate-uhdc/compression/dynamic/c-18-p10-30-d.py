...
def interCheck(point,center,radius1,radius2):
    sign = True
    for p in center:
        if sqrt((point[0]-p[0])**2+(point[1]-p[1])**2+(point[2]-p[2])**2) <= (radius1+radius2+0.1):
            sign = False
            break
    return sign
...
radius = 5
while True:
    disX = random.uniform(radius+0.1, 100-radius-0.1)
    disY = random.uniform(radius+0.1, 100-radius-0.1)
    disZ = random.uniform(radius+0.1, 100-radius-0.1)
    if len(center10)==0:
...
# job
mdb.Job(name='p30c', model='Model-1', description='', type=ANALYSIS, 
